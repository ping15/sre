...
class CourseTemplateUpdateSerializer(serializers.ModelSerializer):
    ...
    num_questions = MappingField(
        field=serializers.IntegerField(label="考题数量"), mapping={"": None}, required=False, allow_null=True)
    passing_score = MappingField(
        serializers.IntegerField(label="过线分数"), mapping={"": None}, required=False, allow_null=True)
